...
typedef struct M3Memory
{
    ...
    // RN-WEBASSEMBLY PATCH (non-moving memory): when non-null, `mallocated`
    // points into this mmap reservation; growth commits pages in place and
    // the block never relocates, so JS views over the memory stay valid.
    void *                  reservation;
...
typedef struct M3Runtime
{
    ...
    // RN-WEBASSEMBLY PATCH: previously strace-only; now also drives the
    // d_m3MaxCallDepth native-stack recursion guard (see m3_config.h).
    u32                     callDepth;
