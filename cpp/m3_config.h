...
// RN-WEBASSEMBLY PATCH: wasm3 recurses the native C stack once per wasm
// call, with no guard -- deeply recursive wasm segfaults the host before the
// operand-stack limit triggers. Cap the interpreter call depth; exceeding it
// traps with m3Err_trapStackOverflow instead of crashing the process.
# ifndef d_m3MaxCallDepth
#   define d_m3MaxCallDepth                     1000
# endif

// RN-WEBASSEMBLY PATCH: non-moving linear memory. Reserve address space for
// maxPages up front (mmap PROT_NONE, costing no RAM) and commit on grow, so
// memory.grow never relocates the block. Eliminates the stale-JS-view bug
// class (glue caches, heap-held entropy views, app-held buffers). Falls
// back to realloc when reservation fails or on non-POSIX platforms.
# ifndef d_m3EnableNonMovingMemory
#   define d_m3EnableNonMovingMemory            1
# endif
