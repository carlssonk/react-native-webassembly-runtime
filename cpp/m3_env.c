...
// RN-WEBASSEMBLY PATCH: non-moving linear memory (see m3_config.h).
#if d_m3EnableNonMovingMemory && (defined(__APPLE__) || defined(__linux__) || defined(__ANDROID__))
#  include <sys/mman.h>
#  define M3_NONMOVING_POSIX 1
#endif
...
#ifdef M3_NONMOVING_POSIX
    if (i_runtime->memory.reservation)
    {
        munmap (i_runtime->memory.reservation, i_runtime->memory.reservationBytes);
        i_runtime->memory.reservation = NULL;
...
#ifdef M3_NONMOVING_POSIX
        // RN-WEBASSEMBLY PATCH: reserve address space for maxPages on the
        // first allocation; growth then commits pages in place and the block
        // never relocates. Reservation failure falls back to realloc below.
        if (!memory->reservation && !memory->mallocated)
        {
            size_t reserveBytes = (size_t) memory->maxPages * d_m3MemPageSize + sizeof (M3MemoryHeader);
            if (io_runtime->memoryLimit)
                reserveBytes = M3_MIN (reserveBytes, (size_t) io_runtime->memoryLimit + sizeof (M3MemoryHeader));

            void* reservation = mmap (NULL, reserveBytes, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
            if (reservation != MAP_FAILED)
            {
                memory->reservation = reservation;
                memory->reservationBytes = reserveBytes;
                memory->committedBytes = 0;
            }
        }

        if (memory->reservation)
        {
            _throwif (m3Err_wasmMemoryOverflow, numBytes > memory->reservationBytes);

            if (numBytes > memory->committedBytes)
            {
                // Fresh anonymous pages are zero-filled; memory only grows.
                _throwif (m3Err_mallocFailed,
                          mprotect (memory->reservation, numBytes, PROT_READ | PROT_WRITE) != 0);
                memory->committedBytes = numBytes;
            }

            memory->mallocated = (M3MemoryHeader *) memory->reservation;
...
#ifdef RNWASM_LOG_RESIZE
            fprintf (stderr, "[resize] %zu bytes committed in reservation %p (non-moving)\n",
                     numBytes, memory->reservation);
#endif
            return result;
        }
#endif // M3_NONMOVING_POSIX
...
#ifdef RNWASM_LOG_RESIZE
        void* rnwasmOldPtr = (void*) memory->mallocated;
#endif
        void* newMem = m3_Realloc ("Wasm Linear Memory", memory->mallocated, numBytes, numPreviousBytes);
        _throwifnull(newMem);
#ifdef RNWASM_LOG_RESIZE
        fprintf(stderr, "[resize] %zu -> %zu bytes, %p -> %p%s\n",
                numPreviousBytes, numBytes, rnwasmOldPtr, newMem,
                rnwasmOldPtr && rnwasmOldPtr != newMem ? "  RELOCATED" : "");
#endif
