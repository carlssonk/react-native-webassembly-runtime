...
d_m3Op  (Call)
{
    ...
    // RN-WEBASSEMBLY PATCH: native-stack recursion guard (see m3_config.h).
    IM3Runtime rt = _mem->runtime;
    if (M3_UNLIKELY(rt->callDepth >= d_m3MaxCallDepth))
    {
        m3ret_t overflow = m3Err_trapStackOverflow;
        pushBacktraceFrame ();
        forwardTrap (overflow);
    }
    ...
    if (M3_LIKELY(tableIndex < module->table0Size))
    {
        ...
        if (M3_LIKELY(function))
        {
            if (M3_LIKELY(type == function->funcType))
            {
                ...
                // RN-WEBASSEMBLY PATCH: native-stack recursion guard.
                if (M3_LIKELY(not r) && M3_UNLIKELY(_mem->runtime->callDepth >= d_m3MaxCallDepth))
                    r = m3Err_trapStackOverflow;
                else if (M3_LIKELY(not r))
                {
                    IM3Runtime rt = _mem->runtime;
                    rt->callDepth++;
...
# endif
    ...
    // RN-WEBASSEMBLY PATCH: refresh unconditionally, not only on trap. A JS
    // import may reenter wasm exports (the recursion this function already
    // supports) and grow/relocate linear memory; continuing with the stale
    // _mem after a successful import corrupts all subsequent accesses in
    // this frame. Harmless upstream (raw imports there cannot grow memory).
    _mem = memory->mallocated;
    if (M3_UNLIKELY(possible_trap)) {
        pushBacktraceFrame ();
    }
