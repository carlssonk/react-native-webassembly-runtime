...
class JSCRuntime : public jsi::Runtime {
 ...
 private:
  ...
  // Harness patch: BigInts are primitive JSValueRefs like Symbols, so they
  // reuse the JSCSymbolValue protect/unprotect machinery.
  static JSValueRef bigintRef(const jsi::BigInt &b);
  jsi::BigInt createBigInt(JSValueRef ref) const;
  ...
  std::string desc_;
  std::vector<jsi::Value> microtaskQueue_;
#ifndef NDEBUG
...
void JSCRuntime::queueMicrotask(const jsi::Function &callback) {
  microtaskQueue_.emplace_back(*this, callback);
}

bool JSCRuntime::drainMicrotasks(int maxMicrotasksHint) {
  // JSC services its own promise-job queue on returning to the C API; this
  // only drains jobs enqueued through the JSI queueMicrotask API.
  while (!microtaskQueue_.empty()) {
    std::vector<jsi::Value> queue;
    queue.swap(microtaskQueue_);
    for (auto &task : queue) {
      task.asObject(*this).asFunction(*this).call(*this);
    }
  }
  return true;
}
...
#endif
{
  // Harness patch: this pointer-value class is shared with BigInts (both are
  // primitive JSValueRefs needing protect/unprotect).
  assert(smellsLikeES6Symbol(ctx_, sym_) || JSValueIsBigInt(ctx_, sym_));
...
jsi::Runtime::PointerValue *JSCRuntime::cloneBigInt(
  ...
  const JSCSymbolValue *bigint = static_cast<const JSCSymbolValue *>(pv);
  return makeSymbolValue(bigint->sym_);
}
...
namespace {
// Harness patch: JSStringCreateWithUTF8CString truncates at embedded NULs
// (legal in wasm export names). Convert UTF-8 -> UTF-16 by length instead.
std::vector<JSChar> utf8ToUtf16(const uint8_t *utf8, size_t length) {
  ...
  for (size_t i = 0; i < length;) {
    ...
    if (a < 0x80) {
      cp = a;
      i += 1;
    } else if (a < 0xe0 && i + 1 < length) {
      cp = ((a & 0x1f) << 6) | (utf8[i + 1] & 0x3f);
      i += 2;
    } else if (a < 0xf0 && i + 2 < length) {
      cp = ((a & 0x0f) << 12) | ((utf8[i + 1] & 0x3f) << 6) |
          (utf8[i + 2] & 0x3f);
      i += 3;
    } else if (i + 3 < length) {
      cp = ((a & 0x07) << 18) | ((utf8[i + 1] & 0x3f) << 12) |
    ...
    if (cp < 0x10000) {
      out.push_back(static_cast<JSChar>(cp));
    } else {
      cp -= 0x10000;
      out.push_back(static_cast<JSChar>(0xd800 + (cp >> 10)));
      out.push_back(static_cast<JSChar>(0xdc00 + (cp & 0x3ff)));
    }
  }
  return out;
}
...
jsi::PropNameID JSCRuntime::createPropNameIDFromUtf8(
    const uint8_t *utf8,
    size_t length) {
  auto utf16 = utf8ToUtf16(utf8, length);
  static const JSChar kEmpty = 0;
  JSStringRef strRef = JSStringCreateWithCharacters(
      utf16.empty() ? &kEmpty : utf16.data(), utf16.size());
  auto res = createPropNameID(strRef);
  JSStringRelease(strRef);
  return res;
}
...
jsi::BigInt JSCRuntime::createBigInt(JSValueRef ref) const {
  return make<jsi::BigInt>(makeSymbolValue(ref));
}

JSValueRef JSCRuntime::bigintRef(const jsi::BigInt &b) {
  return static_cast<const JSCSymbolValue *>(getPointerValue(b))->sym_;
}

jsi::BigInt JSCRuntime::createBigIntFromInt64(int64_t value) {
  JSValueRef exc = nullptr;
  JSValueRef res = JSBigIntCreateWithInt64(ctx_, value, &exc);
  checkException(exc);
  return createBigInt(res);
}

jsi::BigInt JSCRuntime::createBigIntFromUint64(uint64_t value) {
  JSValueRef exc = nullptr;
  JSValueRef res = JSBigIntCreateWithUInt64(ctx_, value, &exc);
  checkException(exc);
  return createBigInt(res);
}

bool JSCRuntime::bigintIsInt64(const jsi::BigInt &b) {
  JSValueRef exc = nullptr;
  JSValueRef roundtrip =
      JSBigIntCreateWithInt64(ctx_, JSValueToInt64(ctx_, bigintRef(b), &exc), &exc);
  bool eq = JSValueIsStrictEqual(ctx_, bigintRef(b), roundtrip);
  checkException(exc);
  return eq;
}

bool JSCRuntime::bigintIsUint64(const jsi::BigInt &b) {
  JSValueRef exc = nullptr;
  JSValueRef roundtrip = JSBigIntCreateWithUInt64(
      ctx_, JSValueToUInt64(ctx_, bigintRef(b), &exc), &exc);
  bool eq = JSValueIsStrictEqual(ctx_, bigintRef(b), roundtrip);
  checkException(exc);
  return eq;
}

uint64_t JSCRuntime::truncate(const jsi::BigInt &b) {
  JSValueRef exc = nullptr;
  uint64_t res = JSValueToUInt64(ctx_, bigintRef(b), &exc);
  checkException(exc);
  return res;
}

jsi::String JSCRuntime::bigintToString(const jsi::BigInt &b, int radix) {
  if (radix != 10) {
    throw jsi::JSError(*this, "Harness bigintToString only supports radix 10");
  }
  JSValueRef exc = nullptr;
  JSStringRef str = JSValueToStringCopy(ctx_, bigintRef(b), &exc);
  checkException(exc);
  auto result = createString(str);
  JSStringRelease(str);
  return result;
}
...
namespace {
void mutableBufferDeallocator(void * /*bytes*/, void *context) {
...
jsi::ArrayBuffer JSCRuntime::createArrayBuffer(
  ...
  auto *holder = new std::shared_ptr<jsi::MutableBuffer>(buffer);
  JSValueRef exc = nullptr;
  JSObjectRef obj = JSObjectMakeArrayBufferWithBytesNoCopy(
      ctx_,
      buffer->data(),
      buffer->size(),
      &mutableBufferDeallocator,
      holder,
      &exc);
  checkException(exc);
...
jsi::Value JSCRuntime::createValue(JSValueRef value) const {
  ...
  switch (type) {
    ...
    case kJSTypeObject: {
      ...
      //    case kJSTypeSymbol:
    default: {
      if (JSValueIsBigInt(ctx_, value)) {
        return jsi::Value(createBigInt(value));
...
JSValueRef JSCRuntime::valueRef(const jsi::Value &value) {
  ...
  if (value.isUndefined()) {
    ...
    return bigintRef(value.getBigInt(*this));
